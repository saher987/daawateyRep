...
export default function CreateEvent() {
  ...
  // Search by name, phone, or email — not an exact-match lookup, so a user
  // can actually be found without knowing their exact phone number. Was
  // exact-match-only before, which also had a real bug: a user found by
  // email with no phone on file fell back to storing the *typed email
  // string* into owner_phones as if it were a phone number, silently
  // corrupting event.owner_phones. Now a phone-less match is shown but
  // can't be added (disabled, with an explanatory hint) — event ownership
  // is inherently phone-keyed, so there's no way around that requirement
  // for *this* array, but at least it's an honest, visible constraint
  ...
  const addOwner = (u) => {
    if (!u.phone) return;
    if (owners.find(o => o.phone === u.phone)) return; // already added
    setOwners(prev => [...prev, { phone: u.phone, name: u.full_name, email: u.email }]);
    setOwnerSearchInput("");
